...
CREATE TABLE "MEMBER" (
	...
	"ROLE_NO" NUMBER DEFAULT 3 NOT NULL,
...
COMMENT ON COLUMN "MEMBER"."ROLE_NO" IS '작성, 조회, 수정, 삭제 권한 부여';
...
COMMENT ON COLUMN "REPORT"."REPORT_TYPE" IS '1이면 게시글 2면 댓글';
...
COMMENT ON COLUMN "TRASH"."POST_NO" IS '내용번호';
...
CREATE TABLE "ROLE" (
	"ROLE_NO" NUMBER NOT NULL PRIMARY KEY,
	"ROLE_NAME" VARCHAR2(30) DEFAULT 1 NOT NULL
);

COMMENT ON COLUMN "ROLE"."ROLE_NAME" IS '1. 조회, 2.게시글작성,수정,삭제 3.댓글 작성,수정,삭제 4.관리자';
...
COMMENT ON COLUMN "SUGGESTION"."TRASH_NO" IS '쓰레기번호';
...
ALTER TABLE MEMBER ADD CONSTRAINT FK_M_REF_ROLE_NO FOREIGN KEY(ROLE_NO) REFERENCES ROLE(ROLE_NO);
...
ALTER TABLE SUGGESTION ADD CONSTRAINT FK_SG_REF_TRASH_NO FOREIGN KEY(TRASH_NO) REFERENCES TRASH(TRASH_NO);
ALTER TABLE SUGGESTION ADD CONSTRAINT FK_SG_REF_USER_NO FOREIGN KEY(USER_NO) REFERENCES MEMBER(USER_NO);

ALTER TABLE TRASH ADD CONSTRAINT FK_T_REF_POST_NO FOREIGN KEY(POST_NO) REFERENCES TRASH_POST(TRASH_POST_NO);
